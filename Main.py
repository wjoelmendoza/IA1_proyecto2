...
from neural.Util.utils import get_Dataset,generar_json_deps_mun,tratamiento_de_datos,make_client_Dataset
import numpy as np
import codecs, json 
#-----------------------------
...
_hiperparametros = [
    ...
    [10000,5000,8000,6000,7500,12500,5000,9500,12500,10000],
...
def predecir(genero,edad,anio,dep,mun):#return y_hat?
    ...
    train_X,train_Y = make_client_Dataset(genero,edad,anio,dep,mun)
    train_X2,val_X,test_X,train_Y,val_Y2,test_Y = get_Dataset()
     # Definir los conjuntos de datos
    print("!!!!!!!!!!!!!!!!!!!!!!CREACION DE DATOS!!!!!!!!!!!!!!!!!!!")
    ...
    capas1 = [test_set.n, 6,6,6,1]   
    # Se define el modelo
    nn1 = NN_Model(None, capas1, alpha=0.3, iterations=1000, lambd=0, keep_prob=1,paras=newpesos,flag=ENTRENO)
    #nn1 = NN_Model(None, capas1, alpha=1000, iterations=-777, lambd=0, keep_prob=1,paras=newpesos,flag=ENTRENO)
    #nn2 = NN_Model(train_set, capas1, alpha=0.3, iterations=50000, lambd=0.7, keep_prob=1)
    ...
    print(exactitud)
    print(y_hat)
    if y_hat[0][0] > 0.5:#0->traslado 1-> activo
        return "Activo"
    else:
        return "Traslado"
